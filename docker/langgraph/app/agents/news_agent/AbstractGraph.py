...
from scrapegraphai.models.xai import XAI
from scrapegraphai.utils.logging import set_verbosity_info, set_verbosity_warning, get_logger
from scrapegraphai.telemetry.telemetry import log_graph_execution
...
class AbstractGraph(ABC):
    ...
    def _create_llm(self, llm_config: dict) -> object:
        ...
        - Keeps `model_tokens` INTERNAL ONLY (used for chunking etc.)
        - Preserves supported kwargs for Bedrock in `model_kwargs`
        - Avoids leaking unsupported keys into the provider payload
        """
        ...
        # ---- Fast-path: external model instance provided ----
        if "model_instance" in llm_params:
            # NOTE: model_tokens must be provided alongside model_instance
            try:
        ...
        known_providers = {
            "openai", "azure_openai", "google_genai", "google_vertexai", "ollama",
            "oneapi", "nvidia", "groq", "anthropic", "bedrock", "mistralai",
            "hugging_face", "deepseek", "ernie", "fireworks", "clod", "togetherai", "xai",
        }
        ...
        if "/" in llm_params["model"]:
            ...
            if not possible_providers:
                raise ValueError(
                    f'Provider {llm_params.get("model_provider")} is not supported. '
                    "If possible, try to use a model instance instead."
                )
            llm_params["model_provider"] = possible_providers[0]
            print(
                f"Found providers {possible_providers} for model {llm_params['model']}, "
                f"using {llm_params['model_provider']}.\n"
        ...
        # ---- INTERNAL ONLY: determine token budget and REMOVE from params ----
        ...
        # ---- Build provider-safe kwargs and init model ----
        try:
            # Providers handled by LangChain's init_chat_model
            if llm_params["model_provider"] not in {
                "oneapi", "nvidia", "ernie", "deepseek", "togetherai", "clod", "xai"
            }:
                # Special care for Bedrock: curate supported kwargs only
                if llm_params["model_provider"] == "bedrock":
                    # Whitelist of safe generation params for Bedrock Chat models
                    allowed_kw = {
                        "temperature", "max_tokens", "top_p", "top_k",
                        "stop", "stop_sequences"  # some wrappers use 'stop' vs 'stop_sequences'
                    }
                    mk = {}
                    for k in list(llm_params.keys()):
                        if k in allowed_kw:
                            mk[k] = llm_params.pop(k)
                    # Preserve/merge with existing model_kwargs if caller supplied any
                    existing_mk = llm_params.get("model_kwargs", {})
                    if existing_mk and not isinstance(existing_mk, dict):
                        existing_mk = {}
                    # Merge (explicit keys take precedence)
                    merged = {**existing_mk, **mk}
                    llm_params["model_kwargs"] = merged

                # IMPORTANT: by here, `llm_params` contains NO 'model_tokens'
                # and only safe keys in model_kwargs for Bedrock.
                with warnings.catch_warnings():
                    warnings.simplefilter("ignore")
                    return init_chat_model(**llm_params)

            # ---- Special providers path (unchanged, just ensure model_tokens is gone) ----
